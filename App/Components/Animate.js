...
import { StyleSheet, StatusBar, Dimensions, Text, Animated, Easing } from 'react-native';
...
class AnimatedGradient extends Component {
  ...
  startAnimation = () => {
    ...
    Animated.parallel([color0, color1].map(animatedColor => Animated.timing(animatedColor, {
  ...
  render() {
    ...
    const interpolatedColors = [color0, color1]
      .map((animatedColor, index) => animatedColor.interpolate({
